...
//  console.log(state.options.attributes)
  let cubeVertexArray = new Float32Array(state.options.attributes[name].data.flat());
...
const recordRenderPass = async function (state, CE) {
  ...
  const commandEncoder = CE || state.commandEncoder|| device.createCommandEncoder();
  ...
  for (var key in state.options.attributes) {
    passEncoder.setVertexBuffer(i, updateAttributes(state, device, key));
  }
  // if (state.options?.attributes?.position) {
  //   passEncoder.setVertexBuffer(0, updateAttributes(state, device, 'position'));
  // }
...
async function makePipeline(state) {
  ...
  if (state.options.attributeBuffers) {
    ...
    //   }
    // ]
    // console.log(pipelineDesc.vertex.buffers)
    // console.log(state.options.attributes)
    pipelineDesc.vertex.buffers = []
...
async function init(options={}) {
  ...
  async function initDrawCall (options) {
    ...
    function draw(newScope, commandEncoder) {
      ...
      recordRenderPass(state, commandEncoder);
      return draw;
    }
    ...
    await createRenderPasses(state, options);
    return draw
  }
}
