...
function makeUniformWrite(size, device) {
  console.log(size)
  return function (a, uniformBuffer) { device.queue.writeBuffer(uniformBuffer, size, a.buffer, a.byteOffset, a.byteLength)}
}
...
//    stuff[key] = makeUniformWrite(size, device)
...
async function makeBindGroup(state, options) {
  ...
  let {device, pipeline} = state;
    ...
    let stuff = bindUniforms(options, device)
    state.uniformBuffer = stuff[0]
    state.uniforms = stuff[1]
...
const recordRenderPass = async function (state) {
  ...
  //add uniforms to dict and only upload if change
  if (state.options?.uniforms?.modelViewProjectionMatrix) {
    ...
    if (state.options?.uniforms?.projection) {

      const c = state.options.uniforms.projection()
      const b = state.options.uniforms.lookAt()
      const a = state.options.uniforms.fromRotation({tick: Date.now()})

      // state.uniforms['fromRotation'](a, state.uniformBuffer)
      // state.uniforms['lookAt'](b, state.uniformBuffer)
      // state.uniforms['projection'](c, state.uniformBuffer)
      device.queue.writeBuffer(state.uniformBuffer, 0, a.buffer, 0, 16 * 4)
      device.queue.writeBuffer(state.uniformBuffer, 64, b.buffer, 0, 16 * 4)
      device.queue.writeBuffer(state.uniformBuffer, 128, c.buffer, 0, 16 * 4)
