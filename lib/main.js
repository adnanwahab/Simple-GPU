...
async function makeBindGroup(state, options) {
  ...
  let keys = Object.keys(options.uniforms || {});
  let values = Object.values(options.uniforms || {});
  values.map(fn => isFunction(fn) && fn(context)).reduce((a, b) => {
    return Array.isArray(a) && a.length + b;
  }, [])

  let {device, pipeline} = state;
    //TODO Construct dynamically
    const uniformBufferSize = 3 * 4 * 16; // 4x4 matrix
...
const recordRenderPass = async function (state) {
  ...
  if (state.options?.uniforms?.modelViewProjectionMatrix) {
    ...
    if (state.options?.uniforms?.projection) {
      const a = state.options.uniforms.projection()
      const b = state.options.uniforms.lookAt()
      const c = state.options.uniforms.fromRotation({tick: Date.now()})
      const matrixBuffer = state.options.uniforms.matrixBuffer();
      device.queue.writeBuffer(state.uniformBuffer, 0, a.buffer, 0, 16 * 4)
      device.queue.writeBuffer(state.uniformBuffer, 16, b.buffer, 0, 16 * 4)
      device.queue.writeBuffer(state.uniformBuffer, 32, c.buffer, 0, 16 * 4)


      //console.log(matrixBuffer)
      device.queue.writeBuffer(state.uniformBuffer, 0, matrixBuffer.buffer, 0, 3 * 16 * 4)
  }
