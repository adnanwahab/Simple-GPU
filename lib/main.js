...
const recordRenderPass = async function (state) {
  ...
  if (state.options?.uniforms?.modelViewProjectionMatrix) {
    ...
    if (state.options?.uniforms?.projection) {
      ...
      device.queue.writeBuffer(state.uniformBuffer, 0, a.buffer, a.byteOffset, a.byteLength)
      device.queue.writeBuffer(state.uniformBuffer, 64,  b.buffer, b.byteOffset, b.byteLength)
      device.queue.writeBuffer(state.uniformBuffer, 128, c.buffer, c.byteOffset, c.byteLength)
  }
