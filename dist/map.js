...
(() => {
  // lib/utils.js
  var addMouseEvents = function(canvas, data2) {
    canvas.addEventListener("mousemove", (event) => {
      let x = event.pageX;
      let y = event.pageY;
      data2.mouseX = x / event.target.clientWidth;
      data2.mouseY = y / event.target.clientHeight;
    });
  };
  ...
  function makeBuffer(device, size = 4, usage, data2, type2) {
    const buffer2 = device.createBuffer({
      size,
      mappedAtCreation: true,
      usage: GPUBufferUsage[usage]
    });
    new type2(buffer2.getMappedRange()).set(data2);
    buffer2.unmap();
    return buffer2;
  }
  ...
  async function init(options = {}) {
    ...
    function initDrawCall(options2) {
      ...
      draw.draw = draw;
      draw.state = localState;
      draw.swapAttributeBuffer = function(data2, i) {
        localState.options.attributeBufferData[i] = data2;
      };
      return draw;
    }
  }
  ...
  function buffer(array2) {
    if (!(this instanceof buffer))
      return new buffer(array2);
    this.array = array2;
  }
  function prop(name) {
  ...
  function attribute(data2, offset, format) {
    return {
      data: data2,
      offset,
  ...
  var streamGeometryType = {
    ...
    MultiPoint: function(object, stream) {
      var coordinates2 = object.coordinates, i = -1, n = coordinates2.length;
      while (++i < n)
        object = coordinates2[i], stream.point(object[0], object[1], object[2]);
    },
    ...
    function clipLine(stream) {
      ...
      return {
        ...
        point: function(lambda, phi) {
          var point1 = [lambda, phi], point2, v = visible(lambda, phi), c2 = smallRadius ? v ? 0 : code(lambda, phi) : v ? code(lambda + (lambda < 0 ? pi : -pi), phi) : 0;
          if (!point0 && (v00 = v0 = v))
            stream.lineStart();
          if (v !== v0) {
  ...
  function resample(project, delta2) {
    function resampleLineTo(x02, y02, lambda0, a0, b0, c0, x12, y12, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * delta2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c2 = c0 + c1, m2 = sqrt(a * a + b * b + c2 * c2), phi2 = asin(c2 /= m2), lambda2 = abs2(abs2(c2) - 1) < epsilon || abs2(lambda0 - lambda1) < epsilon ? (lambda0 + lambda1) / 2 : atan2(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x02, dy2 = y2 - y02, dz = dy * dx2 - dx * dy2;
        if (dz * dz / d2 > delta2 || abs2((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
          resampleLineTo(x02, y02, lambda0, a0, b0, c0, x2, y2, lambda2, a /= m2, b /= m2, c2, depth, stream);
          stream.point(x2, y2);
          resampleLineTo(x2, y2, lambda2, a, b, c2, x12, y12, lambda1, a1, b1, c1, depth, stream);
        }
      }
    }
    return function(stream) {
  ...
  function projectionMutator(projectAt) {
    var project, k = 150, x = 480, y = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x02 = null, y02, x12, y12, postclip = identity_default, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
    ...
    projection3.clipExtent = function(_) {
      return arguments.length ? (postclip = _ == null ? (x02 = y02 = x12 = y12 = null, identity_default) : clipRectangle(x02 = +_[0][0], y02 = +_[0][1], x12 = +_[1][0], y12 = +_[1][1]), reset()) : x02 == null ? null : [[x02, y02], [x12, y12]];
    };
  ...
  var data = csv2(`https://raw.githubusercontent.com/stackgpu/Simple-GPU/main/data/311.csv`).then((d) => {
    d.forEach((d2) => {
      coordinates.push(
        canvasToClipSpace(projection2([
          +d2.Longitude,
          +d2.Latitude
        ]))
      );
  ...
  async function main() {
    ...
    quadBuffer.unmap();
    function makeBuffer2(size = particlesCount, flag = 1, log2) {
