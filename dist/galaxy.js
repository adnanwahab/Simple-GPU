...
(() => {
  ...
  function makeBuffer(device, size = 4, usage, data, type2) {
    ...
    new type2(buffer2.getMappedRange()).set(data);
    buffer2.unmap();
    return buffer2;
  }
  ...
  async function makePipeline(state2) {
    ...
    let pipelineDesc = {
      ...
      fragment: {
        ...
        targets: state2.options.targets ? state2.options.targets.map((format2) => {
  ...
  function attribute(data, offset, format2) {
    return {
      data,
      offset,
      format: `float32x${format2}`
    };
  }
  ...
  function projectionMutator(projectAt) {
    var project, k = 150, x = 480, y = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x02 = null, y02, x12, y12, postclip = identity_default2, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
    ...
    projection3.clipExtent = function(_) {
      return arguments.length ? (postclip = _ == null ? (x02 = y02 = x12 = y12 = null, identity_default2) : clipRectangle(x02 = +_[0][0], y02 = +_[0][1], x12 = +_[1][0], y12 = +_[1][1]), reset()) : x02 == null ? null : [[x02, y02], [x12, y12]];
    };
  ...
  fetch(`https://raw.githubusercontent.com/stackgpu/Simple-GPU/main/src/bye.txt`).then((response) => {
    ...
    let xScale = linear2().domain([min(x), max(x)]).range([-1, 1]);
    let yScale = linear2().domain([min(y), max(y)]).range([-1, 1]);
    console.log(x);
    let a = x.map(xScale);
    let b = y.map(yScale);
    main(a, b);
